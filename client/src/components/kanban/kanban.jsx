...
const Kanban = () => {
  ...
  const onDropHandle = (columnId) => (taskId, nextTask, prevTask) =>
    changeTask(taskId, columnId, nextTask, prevTask)
  ...
  return (
    <>
          ...
          <DragDrop>
            {columns.map((column) => {
              return (
                <Droppable
                  key={column._id}
                  onDrop={onDropHandle(column._id)}
                  className="col">
      ...
      <Modal show={modalShow} onClose={modalCloseHandler}>
        ...
        <ModalBody>
          Вы действительно хотите удалить задачу &quot;{taskForDeletion?.title}&quot;?
        </ModalBody>
        <ModalFooter>
          <button
            type="button"
