...
export const updateTask = (payload) => async (dispatch, getState) => {
  ...
  try {
    ...
    const prevColumn = getState().tasks.entities.find(
      (item) => item._id === payload._id
    )?.column
    let tasks = getState().tasks.entities.filter(
      (item) => item.column === column && item._id !== payload._id
    )
...
export const createTask = (task) => async (dispatch, getState) => {
  ...
  try {
    ...
    const sort = getState()
      .tasks.entities.filter((item) => item.column === column)
      .reduce((sum, item) => {
        if (item.sort >= sum) return item.sort + 1
        return sum
      }, 0)
